...
describe('DateTimeFormat Attribute', function(): void {
    it('does NOT format DateTime in toArray (keeps DateTime object)', function(): void {
        ...
        $dto = new class($dateTime) extends SimpleDto {
    ...
    it('formats DateTime with custom format in toJson', function(): void {
        $dto = new class(new DateTimeImmutable('2024-06-25 14:30:00')) extends SimpleDto {
            public function __construct(
    ...
    it('works with DateTime (not just DateTimeImmutable) in toJson', function(): void {
        $dto = new class(new DateTime('2024-01-15 10:30:00')) extends SimpleDto {
            public function __construct(
                #[DateTimeFormat('Y-m-d H:i:s')]
    ...
    it('formats DateTime with nested DTOs in toJson', function(): void {
        $innerDto = new class(new DateTimeImmutable('2024-01-15 10:30:00')) extends SimpleDto {
            public function __construct(
                #[DateTimeFormat('Y-m-d')]
                public readonly DateTimeImmutable $date,
            ) {}
        ...
        $outerDto = new class($innerDto) extends SimpleDto {
            public function __construct(
    ...
    it('preserves DateTime objects through serialize/unserialize', function(): void {
        ...
        $className = 'TestSerializableDto_' . uniqid();
        // @phpstan-ignore-next-line (eval is necessary to create named class for serialization test)
        eval("
            class {$className} extends event4u\\DataHelpers\\SimpleDto\\SimpleDto {
                public function __construct(
                    #[event4u\\DataHelpers\\SimpleDto\\Attributes\\DateTimeFormat('Y-m-d H:i:s')]
                    public readonly DateTimeImmutable \$createdAt,
                ) {}
            }
        ");
