...
describe('Laravel Validation Integration', function(): void {
    describe('Rule Generation for Laravel', function(): void {
        it('generates Laravel-compatible rule for Exists attribute', function(): void {
            $attribute = new Exists('users', 'id');
            expect($attribute->rule())->toBe('exists:users,id');
        });

        it('generates Laravel-compatible rule for Exists with connection', function(): void {
            $attribute = new Exists('users', 'id', 'mysql');
            expect($attribute->rule())->toBe('exists:mysql.users,id');
        });

        it('generates Laravel-compatible rule for Unique attribute', function(): void {
            $attribute = new Unique('users', 'email');
            expect($attribute->rule())->toBe('unique:users,email');
        });

        it('generates Laravel-compatible rule for Unique with ignore', function(): void {
            $attribute = new Unique('users', 'email', 123);
            expect($attribute->rule())->toBe('unique:users,email,123,id');
        });

        it('generates Laravel-compatible rule for Unique with custom id column', function(): void {
            $attribute = new Unique('users', 'email', 123, 'user_id');
            expect($attribute->rule())->toBe('unique:users,email,123,user_id');
        });

        it('generates Laravel-compatible rule for In attribute', function(): void {
            $attribute = new In(['admin', 'user', 'guest']);
            expect($attribute->rule())->toBe('in:admin,user,guest');
        });

        it('generates Laravel-compatible rule for NotIn attribute', function(): void {
            $attribute = new NotIn(['banned', 'deleted']);
            expect($attribute->rule())->toBe('not_in:banned,deleted');
        });

        it('generates Laravel-compatible rule for Between attribute', function(): void {
            $attribute = new Between(18, 65);
            expect($attribute->rule())->toBe('between:18,65');
        });

        it('generates Laravel-compatible rule for Confirmed attribute', function(): void {
            $attribute = new Confirmed();
            expect($attribute->rule())->toBe('confirmed');
        });

        it('generates Laravel-compatible rule for File attribute', function(): void {
            $attribute = new File();
            expect($attribute->rule())->toBe('file');
        });

        it('generates Laravel-compatible rules for File with max size', function(): void {
            $attribute = new File(maxSize: 10240);
            expect($attribute->rule())->toBe(['file', 'max:10240']);
        });

        it('generates Laravel-compatible rules for File with min and max size', function(): void {
            $attribute = new File(maxSize: 10240, minSize: 1024);
            expect($attribute->rule())->toBe(['file', 'max:10240', 'min:1024']);
        });

        it('generates Laravel-compatible rules for Image attribute', function(): void {
            $attribute = new Image();
            expect($attribute->rule())->toBe(['image']);
        });

        it('generates Laravel-compatible rules for Image with mimes', function(): void {
            $attribute = new Image(mimes: ['jpg', 'png', 'gif']);
            expect($attribute->rule())->toBe(['image', 'mimes:jpg,png,gif']);
        });

        it('generates Laravel-compatible rules for Image with max size', function(): void {
            $attribute = new Image(maxSize: 2048);
            expect($attribute->rule())->toBe(['image', 'max:2048']);
        });

        it('generates Laravel-compatible rules for Image with dimensions', function(): void {
        ...
        it('generates Laravel-compatible rule for Mimes attribute', function(): void {
            $attribute = new Mimes(['pdf', 'doc', 'docx']);
            expect($attribute->rule())->toBe('mimes:pdf,doc,docx');
        });

        it('generates Laravel-compatible rule for MimeTypes attribute', function(): void {
        ...
        it('generates Laravel-compatible rule for Size attribute', function(): void {
            $attribute = new Size(10);
            expect($attribute->rule())->toBe('size:10');
        });

        it('generates Laravel-compatible rule for Same attribute', function(): void {
            $attribute = new Same('password');
            expect($attribute->rule())->toBe('same:password');
        });

        it('generates Laravel-compatible rule for Different attribute', function(): void {
            $attribute = new Different('email');
            expect($attribute->rule())->toBe('different:email');
        });

        it('generates Laravel-compatible rule for StartsWith attribute', function(): void {
            $attribute = new StartsWith(['http://', 'https://']);
            expect($attribute->rule())->toBe('starts_with:http://,https://');
        });

        it('generates Laravel-compatible rule for EndsWith attribute', function(): void {
            $attribute = new EndsWith(['.com', '.org', '.net']);
            expect($attribute->rule())->toBe('ends_with:.com,.org,.net');
        });

        it('generates Laravel-compatible rule for Ip attribute', function(): void {
            $attribute = new Ip();
            expect($attribute->rule())->toBe('ip');
        });

        it('generates Laravel-compatible rule for Ip with IPv4', function(): void {
            $attribute = new Ip(version: 'ipv4');
            expect($attribute->rule())->toBe('ipv4');
        });

        it('generates Laravel-compatible rule for Ip with IPv6', function(): void {
            $attribute = new Ip(version: 'ipv6');
            expect($attribute->rule())->toBe('ipv6');
        });

        it('generates Laravel-compatible rule for Json attribute', function(): void {
            $attribute = new Json();
            expect($attribute->rule())->toBe('json');
        });

        it('generates Laravel-compatible rule for Email attribute', function(): void {
            $attribute = new Email();
            expect($attribute->rule())->toBe('email');
        });

        it('generates Laravel-compatible rule for Url attribute', function(): void {
            $attribute = new Url();
            expect($attribute->rule())->toBe('url');
        });

        it('generates Laravel-compatible rule for Uuid attribute', function(): void {
            $attribute = new Uuid();
            expect($attribute->rule())->toBe('uuid');
        });

        it('generates Laravel-compatible rule for Min attribute', function(): void {
            $attribute = new Min(3);
            expect($attribute->rule())->toBe('min:3');
        });

        it('generates Laravel-compatible rule for Max attribute', function(): void {
