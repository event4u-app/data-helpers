...
class LaravelEndsWithTestDto extends SimpleDto
{
...
describe('EndsWith Attribute - Laravel E2E', function(): void {
    it('passes when string ends with suffix', function(): void {
        $result = LaravelEndsWithTestDto::validate(['domain' => 'example.com']);
        expect($result->isValid())->toBeTrue();
    });

    it('fails when string does not end with suffix', function(): void {
        $result = LaravelEndsWithTestDto::validate(['domain' => 'example.de']);
