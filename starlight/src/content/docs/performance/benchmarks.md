...
- **3.3x faster** than Symfony Serializer for complex mappings
...
- SimpleDto:  ~11μs per operation (depending on casting needs)
- Plain PHP:  ~0.2μs per operation
- Trade-off:  ~43-44x slower, but with automatic type conversion
...
- Symfony:    ~80-98μs per operation
- Benefit:    3.3x faster with better developer experience
```
...
- SimpleDto (with AutoCast): ~11μs   (43x slower than Plain PHP)
- AutoCast overhead:         ~226%

Scenario 2: String types (casting needed)
- SimpleDto (with AutoCast): ~11μs   (44x slower than Plain PHP)
...
|-----------|------|-------------|
| Simple Get | 0.257μs | Get value from flat array |
| Nested Get | 0.317μs | Get value from nested path |
| Wildcard Get | 5.714μs | Get values using single wildcard |
| Deep Wildcard Get | 56.221μs | Get values using multiple wildcards |
| Typed Get String | 0.278μs | Get typed string value |
| Typed Get Int | 0.289μs | Get typed int value |
...
| Operation | Time | Description |
|-----------|------|-------------|
| Simple Set | 0.646μs | Set value in flat array |
| Nested Set | 0.970μs | Set value in nested path |
| Deep Set | 1.102μs | Set value creating new nested structure |
| Multiple Set | 1.511μs | Set multiple values at once |
| Merge | 0.957μs | Deep merge arrays |
| Unset | 0.920μs | Remove single value |
| Multiple Unset | 1.356μs | Remove multiple values |
...
| Operation | Time | Description |
|-----------|------|-------------|
| Simple Mapping | 11.391μs | Map flat structure |
| Nested Mapping | 10.713μs | Map nested structure |
| Auto Map | 8.691μs | Automatic field mapping |
| Map From Template | 10.693μs | Map using template expressions |
...
|--------|-----------|-----------|------------|-------------|
| From Array | 16.834μs<br>&nbsp; | 0.161μs<br>(**104.6x faster**) | 0.211μs<br>(**79.7x faster**) | Our SimpleDto implementation |
| To Array | 22.434μs<br>&nbsp; | - | 0.347μs<br>(**64.7x faster**) | Our SimpleDto toArray() |
| Complex Data | 19.908μs<br>&nbsp; | - | 0.286μs<br>(**69.6x faster**) | Our SimpleDto with complex data |
...
- Plain PHP is **~134x faster** but lacks type safety and validation features
...
|--------|------------|-----------|---------------|-------------|
| Simple Mapping | 11.528μs<br>&nbsp; | 0.088μs<br>(**131.3x faster**) | 3.083μs<br>(**3.7x faster**) | Our DataMapper implementation |
| Nested Mapping | 18.592μs<br>&nbsp; | 0.187μs<br>(**99.3x faster**) | - | Our DataMapper with nested data |
| Template Mapping | 12.222μs<br>&nbsp; | - | - | Our DataMapper with template syntax |
...
- Plain PHP is **~103x faster** but requires manual mapping code for each use case
...
|--------|------------|-----------|-------------------|-------------|
| Template Syntax | 32.175μs<br>&nbsp; | 0.337μs<br>(**95.5x faster**) | 88.817μs<br>(**2.8x slower**) | DataMapper with template syntax |
| Simple Paths | 20.872μs<br>&nbsp; | 0.337μs<br>(**62.0x faster**) | 88.817μs<br>(**4.3x slower**) | DataMapper with simple paths |
...
**Key Insights:**
- DataMapper is **3.3x faster** than Symfony Serializer
...
- MANUAL (no validation):     2.13 μs
- MTIME (auto-validation):    2.21 μs
